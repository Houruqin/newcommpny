...
REDIPS.drag = (function () {
  ...
  init = function (dd) {
    ...
    let self = this,		// assign reference to current object to "self"
  ...
  init_tables = function () {
    let	i, j,				// used in local for loops
  ...
  handler_onmouseup = function (e) {
    let evt = e || window.event,	// define event (FF & IE)
  ...
  set_trc = function (evt) {
    let cell_current,	// define current cell (needed for some test at the function bottom)
  ...
  calculate_cells = function () {
    let i, j,		// local variables used in loops
      row_offset,	// row box
  ...
  clone_object = function () {
    let obj_new = obj.cloneNode(true),	// clone object
  ...
  trash_delete = function () {
    let div_text = 'element',	// div content (inner text)
  ...
  enable_drag = function (enable_flag, div_id) {
    // define local variables
    let i, j,			// local variables used in main loop
  ...
  get_style = function (el, style_name) {
    let val; // value of requested object and property
  ...
  save_content = function (tbl) {
    let query = '',		// define query parameter
