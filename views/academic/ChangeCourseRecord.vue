  ...
  <div class="flex1">
    ...
    <el-card shadow="hover">
      ...
      <el-table stripe class="student-table mt-30" :data="buy_info.data" v-loading="loading" :show-header="true">
        ...
        <el-table-column label="学员姓名" align="center">
          <template slot-scope="scope">
            <!-- <div v-if="$$tools.isAuthority('signDetail')"> -->
              <NameRoute :id="scope.row.stu_id">{{scope.row.studentName}}</NameRoute>
            <!-- </div> -->
        ...
        <el-table-column label="转入课时" prop="operationUserName" align="center"></el-table-column>
        <el-table-column label="转课时间" align="center">
          <template slot-scope="scope">
            <div>
              <div>{{scope.row.createdAt | date('yyyy-MM-dd')}}</div>
            </div>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    async get_data () {
      this.loading = true;
      const params = {
        startTime: this.get_seconde(this.search_info.begin),
        endTime: this.get_seconde(this.search_info.end),
        studentName: this.search_info.name,
        page: this.page_info.page,
        pageNum: this.page_info.page_num
      };
