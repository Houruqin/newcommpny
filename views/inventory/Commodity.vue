    ...
    <div class="flex1">
        ...
        <el-dialog :title="`${operationCommodity == 'add' ? '添加' : '编辑'}物品`" width="720px" center :visible.sync="dialogStatus.addCommodity" :close-on-click-modal="false" @close="dialogClose('addCommodity')">
            ...
            <el-dialog title="添加物品类型" width="500px" center :visible.sync="dialogStatus.commodityType" :close-on-click-modal="false" @close="dialogClose('commodityType_add')" append-to-body>
                <el-form :model="commodityTypeForm" label-width="100px" size="small" :rules="commodityTypeRules" ref="commodityType_add" class="commodity-type-formbox" @submit.native.prevent>
                ...
                <div class="d-f">
                    <div class="flex1">
                        ...
                        <el-form-item label="领取人：" prop="receive_people">
                            <el-select v-model="removeStorageForm.receive_people" placeholder="请输入领取人姓名" filterable remote @change="receivePeopleChange">
                ...
                <div class="d-f">
                    <div>
                        ...
                        <el-form-item label="借用人：" prop="borrow_people">
                            <el-select v-model="borrowForm.borrow_people" placeholder="请输入借用人姓名" filterable remote @change="borrowPeopleChange">
                                <el-option v-for="(item, index) in $store.state.allUser" :key="index" :label="item.name" :value="item.together_id">
                                    <span style="float: left">{{ item.name }}</span>
                                    <span style="float: right; color: #8492a6; font-size: 12px" v-if="item.user_type == 2">学员</span>
                                </el-option>
  ...
  methods: {
    ...
    removeStorageNumValidate (type) {
      return (rule, value, callback, event, e, d) => {
        ...
        if (value > total_num) {
          return callback(new Error('不能超过库存数量'));
        }
      ...
      switch (info.type) {
        ...
        case 'delete':
          this.$confirm('物品删除之后数据不能恢复，请确认进行删除操作！', '提示', {
            ...
            if (info.data.total_num) {
              return this.$message.warning('还有库存，不能删除！');
            }
    ...
    commodityTypeHandle (type, item) {
      if (type == 'edit') {
        ...
        this.commodityTypeSubmit(type, item.id);
      }
    },
    //物品类型删除、禁用
    ...
    borrowPeopleChange (val) {
      this.$store.state.allUser.forEach(v => {
        if (v.together_id == val) {
          this.borrowForm.borrow_people_type = v.user_type;
        }
      });
    },
    //出库 领取人change
      ...
      this.studentCourseLists.forEach(v => {
        if (v.id == val) {
          if (!v.goods.length) {
            ...
            return 0;
          } this.removeStoageBtn.one = false;
    ...
    async submitAddCommodity () {
      ...
      for (let key in this.addCommodityForm) {
        if (key == 'price') {
          if (this.addCommodityForm.use_type == 2) {
      ...
      if (this.operationCommodity == 'edit') {
        params.id = this.addCommodityForm.goods_id;
      }
    ...
    async submitCommodityType (type) {
      ...
      if (this.submitLoading.commodityType) {
        return 0;
      }
      ...
      if (this.commodityTypeStatus == 'edit') {
        params.id = this.commodityTypeForm.id;
      }
      ...
      if (type == 'commodityType_add' && datas) {
        this.addCommodityForm.type = result.list.id;
      }
    ...
    async submitRemoveStorage () {
      if (this.receivePeopleType == 2) {
        if (this.removeStoageBtn.one) {
          return this.$message.warning(this.removeStorageWaringText.one);
        }
        if (this.removeStoageBtn.two) {
          return this.$message.warning(this.removeStorageWaringText.two);
        }
        if (this.removeStoageBtn.three) {
          return this.$message.warning(this.removeStorageWaringText.three);
        }
      }
      ...
      if (this.receivePeopleType == 2) {
        params.student_course = this.removeStorageForm.student_course;
      }
      console.log(params);
