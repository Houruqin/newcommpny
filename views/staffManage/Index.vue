                  ...
                  <el-table-column label="操作" align="center" v-if="$$tools.isAuthority(['quitDeleteStaffs', 'editStaffs', 'enableDisable'])">
                      <template slot-scope="scope">
                        <div class="operable-btn-box">
                          ...
                          <span class="cursor-pointer fc-m" v-if="$$tools.isAuthority('enableDisable') && scope.row.operable" @click="forbidClick(scope.row)">
                              {{scope.row.is_enable == 1 ? '禁用' : '启用'}}
                          </span>
                          <span class="cursor-pointer fc-m" v-if="$$tools.isAuthority('quitDeleteStaffs') && scope.row.status && scope.row.leaveEnable" @click="dimissionClick(scope.row)">离职</span>
                        </div>
                      </template>
                  </el-table-column>
              </el-table>
