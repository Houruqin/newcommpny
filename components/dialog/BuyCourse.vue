...
export default {
  ...
  watch: {
    ...
    buyCourseData (newVal, oldVal) {
      ...
      if (newVal.course_id) {
        ...
        if (newVal.class_pattern == 2) {
          this.courseForm.teacher_id = +newVal.teacher_id;
        }
      }
  ...
  methods: {
    ...
    async submitBuyCourse () {
      if (this.courseForm.lesson_num_already > this.courseForm.lesson_num) {
        return this.$message.warning('已扣课时数不能超过购买课时数!');
      }
      if (this.courseForm.leave_num > this.courseForm.lesson_num) {
        return this.$message.warning('请假次数不能超过购买课时数!');
      }
      if (+this.courseForm.preferential_price > +(this.courseForm.unit_price * this.courseForm.lesson_num + this.courseForm.textbook_price)) {
