...
export default {
    data() {
        return {
            contractData: {
                // institution: {id: 1, name: '益海国际艺术中心'},
                // school: {id: 1, name: '双楠校区'},
                // student: {id: 1, name: '张三'},
                // user: {id: 1, name: '张三'},
                // parent: {id: 1, name: '张家长', mobile: '13241442475'},
                // pay_at: new Date().getTime() / 1000,
                // course: '美术课',
                // lesson_num: 20,
                // given_num: 1,
                // plan_price: 2000,
    ...
    created() {
        if(this.$route.params.contractData) {
            ...
            this.reaplce_path = this.$route.params.replace_path;
    ...
    mounted() {
        ...
        if(this.query) {
            this.$router.replace({path: this.reaplce_path, query: this.query});    //详情购课，需要传参，回到学员详情
        }else {
            this.$router.replace(this.reaplce_path);     //列表购课，直接跳转列表，不需要参数
        }
    }
}
